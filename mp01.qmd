...
# Create folder & download the two Netflix TSVs (saved with .csv names per assignment)
...
```{r verify-downloads, echo=FALSE}
# Show that files exist and have non-zero size
...
```{r import-global, echo=FALSE, message=FALSE, warning=FALSE}
# Packages
...
# Fix "N/A" -> NA in season_title
GLOBAL_TOP_10 <- GLOBAL_TOP_10 |>
  dplyr::mutate(season_title = dplyr::if_else(season_title == "N/A",
                                              NA_character_, season_title))

# Brief confirmation summary (shows in report, code hidden)
GLOBAL_TOP_10 |>
  dplyr::summarise(
    total_rows = dplyr::n(),
    na_in_season_title = sum(is.na(season_title))
  )
```

## Task 3: Import with NA handling (Country table)

```{r import-country, echo=FALSE, message=FALSE, warning=FALSE}
...
# Sanity check: number of literal "N/A" strings left should be 0
zero_na_strings <- sum(COUNTRY_TOP_10$season_title == "N/A", na.rm = TRUE)
zero_na_strings
```
...
```{r q1-countries, echo=FALSE, message=FALSE, warning=FALSE}
# Count distinct countries (your data has `country_name`)
...
```{r q2-nonenglish, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r q3-longest-film, echo=FALSE, message=FALSE, warning=FALSE}
# Look at Films (English & Non-English), keep rows with known runtime, convert to minutes
...
```{r q4-top-hours, echo=FALSE, message=FALSE, warning=FALSE}
# Sum hours across all weeks, per (category, title); pick the top per category
...
```{r q5-longest-run, echo=FALSE, message=FALSE, warning=FALSE}
# Longest *consecutive* run: detect continuity by 7-day gaps
...
```{r q6-short-history, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r q7-squid-hours, echo=FALSE, message=FALSE, warning=FALSE}
# Sum global hours for all seasons of Squid Game in the global table
squid <- GLOBAL_TOP_10 |>
  dplyr::filter(grepl("^TV", category),
...
```{r q8-red-notice-views, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r q9-us-films, echo=FALSE, message=FALSE, warning=FALSE}
# Filter US films (both English & Non-English)
us_films <- COUNTRY_TOP_10 |>
  dplyr::filter(grepl("^Films", category),
                country_iso2 == "US" | country_name == "United States") |>
  dplyr::arrange(show_title, week)

# Debut info per film in the US
debut <- us_films |>
  dplyr::group_by(show_title) |>
  dplyr::summarise(
    debut_week = min(week),
...
```{r q10-debut-most-countries, echo=FALSE, message=FALSE, warning=FALSE}
tv_only <- COUNTRY_TOP_10 |>
  dplyr::filter(grepl("^TV", category))

# Debut week per (show, season)
tv_debut <- tv_only |>
  dplyr::group_by(show_title, season_title) |>
  dplyr::summarise(debut_week = min(week), .groups = "drop")

# Count countries where it charted *in that same debut week*
...
# Optional: show a quick table of the top 10 widest debuts
...
```{r pr_stranger_metrics_calm, echo=FALSE, message=FALSE, warning=FALSE}
...
*Stranger Things* has delivered **`r comma(st_total_hours)`** hours of Top-10 viewing across **`r st_weeks_global`** weeks worldwide. The series reached audiences in **`r st_countries`** countries. Among English-language TV series on Netflix, it ranks **#`r st_rank`** by lifetime Top-10 hours. For context, recent leaders include **`r peer1_name`** with **`r comma(peer1_hours)`** hours and **`r peer2_name`** with **`r comma(peer2_hours)`** hours. Season 5 will arrive with a strong and established global audience.

## Task 6: Press Release 2: Netflix in India

```{r pr_india_prep, echo=FALSE, message=FALSE, warning=FALSE}
# ---- Packages ----
if (!requireNamespace("dplyr", quietly = TRUE))    install.packages("dplyr")
...
# ---- India slice + US titles (to detect India-only hits) ----
...
# ---- India-only hits: longest consecutive run (lifetime) ----
# Detect runs by 7-day gaps within each (show, season)
runs <- india_only %>%
...
# ---- Year-over-year growth proxy: distinct titles that charted in India ----
titles_yearly <- india %>%
...
Over the **last 52 weeks**, **`r NO1_TITLES_52W_LABEL`** different titles reached **#1** in India. Locally resonant storytelling continues to lead the way: the longest India-only Top-10 run belonged to **`r BEST_IO_LABEL`**, holding for **`r BEST_IO_WEEKS_LABEL`** consecutive weeks (**`r BEST_IO_START`** to **`r BEST_IO_END`**). Discovery breadth is growing too—calendar **`r YOY_YEAR_LABEL`** saw a **`r YOY_GROWTH_LABEL`** increase in the number of distinct titles appearing in India’s Top-10. The most recent India-only chart-topper was **`r RIO_TITLE_LABEL`** (week of **`r RIO_WEEK_LABEL`**). Together, these indicators highlight Netflix’s accelerating momentum in India and the expanding appeal of its locally produced hits.

## Task 7: Press Release 3: Global Non-English TV

```{r pr3_noneng_prep, echo=FALSE, message=FALSE, warning=FALSE}
# Packages
if (!requireNamespace("dplyr", quietly = TRUE))    install.packages("dplyr")
if (!requireNamespace("lubridate", quietly = TRUE)) install.packages("lubridate")
if (!requireNamespace("stringr", quietly = TRUE))   install.packages("stringr")
...
# Save a top-10 table for the plot
...
In the **last 52 weeks**, viewers spent **`r NON_HOURS_52W_LABEL`** hours watching Non-English TV on Netflix—**`r NON_SHARE_LABEL`** of all TV Top-10 hours (**`r TV_HOURS_52W_LABEL`** total). Audience choice remained broad, with **`r DISTINCT_NON_52W`** distinct Non-English series appearing in the global Top-10. Leading the wave were **`r TOP1_NAME`** (**`r TOP1_HOURS_LABEL`** hours) and **`r TOP2_NAME`** (**`r TOP2_HOURS_LABEL`** hours). Year over year, Non-English TV Top-10 hours **`r YOY_GROWTH_WORD`** **`r YOY_GROWTH_LABEL`** (calendar **`r YOY_YEAR_LABEL`**), underscoring the sustained global pull of stories told beyond English.

## Extra credit: simple bar chart of the Top 10 Non-English TV titles (last 52 weeks)

```{r pr3_chart, echo=FALSE, message=FALSE, warning=FALSE}
